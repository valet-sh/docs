...
``` bash
valet.sh limit set (cpus|memory) <value> <service-name>
valet.sh limit (get|remove) <service-name> (cpus|memory)
```
...
``` bash
valet.sh limit set cpus 2 opensearch2
```
set the memory limit for a service
``` bash
valet.sh limit set memory 2g opensearch2
```
